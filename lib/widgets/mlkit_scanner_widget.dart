...
class MlkitScanner extends StatefulWidget {
  ...
  static void start(BuildContext context, {required Function(Map<String, dynamic> result) onResult}) {
    showDialog(
      context: context,
      builder: (context) => _ScannerSelectionDialog(
        onSelect: (type) {
          Navigator.pop(context);
          _openScannerScreen(context, type, onResult);
        },
      ),
    );
  }

  static void _openScannerScreen(BuildContext context, ScanType type, Function(Map<String, dynamic> result) onResult) {
    Navigator.push(
      context,
      MaterialPageRoute(
...
class _MlkitScannerState extends State<MlkitScanner> {
  ...
  Widget build(BuildContext context) {
    // This widget can also be used directly if needed, 
    // but the 'start' static method is the preferred way.
    return IconButton(
      icon: const Icon(Icons.qr_code_scanner, color: Colors.blue),
      onPressed: () => MlkitScanner.start(context, onResult: widget.onResult),
...
class _MlkitScannerScreenState extends State<MlkitScannerScreen> {
  ...
  Future<void> _pickFromGallery() async {
    ...
    try {
      ...
      widget.onResult({
        "success": false,
        "scanType": widget.type == ScanType.qr ? "qr" : "ocr",
        "value": "",
        "message": "Error: $e"
      });
    } finally {
